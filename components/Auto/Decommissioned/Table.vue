...
let returnRows = ref<Array<DecommissionedAutoRow>>([]);
let filteredRows = ref<Array<DecommissionedAutoRow>>([]);
...
const items = (row: DecommissionedAutoRow) => [
  [
    {
      label: "Удалить",
  ...
  default:
  {
    ...
    >
      ...
      <template #updated_at-data="{ row }">
        {{ storeUsers.getNormalizedDate(row.autoRow.updated_at) }}
      </template>

      <template #nameOfEquipment-data="{ row }">
        {{ row.autoRow.nameOfEquipment }}
      </template>

      <template #updatedUser-data="{ row }">
        {{ row.autoRow.updatedUser.username }}
      </template>

      <template #autoType-data="{ row }">
        {{ row.autoRow.autoType.name }}
      </template>

...
const props = defineProps({
  rows: {
    type: Array as PropType<DecommissionedAutoRow[]>,
    required: true,
  },
...
const selectedAutoType = ref<Array<string>>([]);
...
const uniqueNameOfEquipment = computed(() => {
  return storeAutos.getUniqueNonEmptyValues(
    rowsAutos.value.filter(
      (row) =>
        row.deleted !== null &&
        props.rows.some((rowData) => rowData.autoRowId === row.id)
    ),
    "nameOfEquipment"
  );
});

const uniqueAutoTypes = computed(() => {
  return ["Лада Гранта", "Газель", "Атлант (бус)", "Атлант (фургон)", "Офис"];
});

const uniqueReason = computed(() => {
  return storeAutos.getUniqueNonEmptyValuesDecommissioned(
...
const filterRows = () => {
  ...
  filteredRows.value = props.rows.filter((row) => {
    return (
      (!selectedAutoType.value.length ||
        selectedAutoType.value.includes(row.autoRow.autoType.name)) &&
      (!selectedNameOfEquipment.value.length ||
        selectedNameOfEquipment.value.includes(row.autoRow.nameOfEquipment)) &&
...
function clearFields() {
  ...
  startingDate.value = "";
  endDate.value = "";
  selected.value.start = new Date(2024, new Date().getMonth(), 1);
...
const rowsAutos = ref<Array<AutoRow>>([]);
onMounted(async () => {
  filterRows();
  rowsAutos.value = await storeAutos.getAutos();
});
    ...
    >
      ...
      >
        <div class="flex items-start space-y-2 flex-col mt-5 text-center">
          <h1>Показать автомобиль</h1>
          <VueMultiselect
            v-model="selectedAutoType"
            :options="uniqueAutoTypes"
            :multiple="true"

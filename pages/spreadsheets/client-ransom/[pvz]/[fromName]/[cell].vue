...
function getCellFromName() {
    if (rowData.value.fromName.trim().length === 12 && isAutoFromName.value === true) {
        let rowCell = originallyRows.value?.filter((row) => row.fromName === rowData.value.fromName);
        if (rowCell) {
    ...
    <div>
        <div v-if="user.role === 'ADMIN'">
            <NuxtLayout name="admin">
                <div v-if="!isLoading" class="mt-3 max-[400px]:mt-20">
                    ...
                    <SpreadsheetsClientRansomTable1 @update-delivery-row="updateDeliveryRow" :rows="filteredRows"
                        :user="user" @delete-row="deleteRow" @open-modal="openModal"
                        @delete-selected-rows="deleteSelectedRows" @update-delivery-rows="updateDeliveryRows"
                        @create-copy-row="createCopyRow" :pvz-link="pvzLink" />
        ...
        <div v-else>
            <NuxtLayout name="user">
                <div v-if="!isLoading" class="mt-3 max-[400px]:mt-20">
                    <div>
                        <SpreadsheetsClientRansomFilters v-if="rows && user.role !== 'PVZ' && user.role !== 'PPVZ'"

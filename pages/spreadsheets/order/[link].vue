  ...
  if (link.startsWith("1")) {
    if (rows.value && flag === "NONE") {
      amountToPaid = rows.value
        ...
        .reduce(
          (acc, value) => acc + Math.ceil(value.amountFromClient1 / 10) * 10,
          0
        );
      ...
      amountToPaid = rows.value
        .filter(
          (value) =>
            value.deliveredPVZ && !value.issued && value.deleted === null
        )
        .reduce(
          (acc, value) => acc + Math.ceil(value.amountFromClient1 / 10) * 10,
          0
    ...
    if (rows.value && flag === "NONE") {
      amountToPaid = rows.value
        .filter((value) => !value.issued && value.deleted === null)
        .reduce(
          (acc, value) => acc + Math.ceil(value.amountFromClient2 / 10) * 10,
          0
        );
    } else if (rows.value && flag === "PVZ") {
      amountToPaid = rows.value
